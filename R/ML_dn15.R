...
bestmtry <- randomForest::tuneRF(
  training[, 2:4], training$d15n.obs, 
  stepFactor = 1.5, improve = 0.01, ntree = 500, trace = FALSE
)
...
colnames(training)
set.seed(1128)
mrf<-caret::train(d15n.obs~prec_mean+tmax_mean+tavg_mean,
...
mcubist <- caret::train(
  d15n.obs ~ prec_mean+tmax_mean+tavg_mean, data = training,
...
#Test Kruskal-Walis
Datamodels <- dplyr::mutate(comp.models,
...
ml1<-lm(comp.obspre.rf$pred..predict.mrf..testing..~ comp.obspre.rf$testing.d15n.obs)
summary(ml1)
#R2
