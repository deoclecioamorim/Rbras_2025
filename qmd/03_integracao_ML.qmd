...
Nessa etapa, o conjunto de dados gerado no Módulo II será utilizado como 
covariáveis para a predição do δ¹⁵N na América do Sul.

```{r importacao_dados}
dn15_var_clim <- readxl::read_excel(here::here("dados", "dn15_var_clim.xlsx"), sheet = 1)
...
sites <- vect(as.data.frame(dn15_var_clim), geom = c("x", "y"), crs = "EPSG:4674")
...
Os resultados do modelo RF com todas as variáveis sugere que mais de 59% da
variância é explicada. O valor de MSE ficou em torno de 3.89.
...
Observe que o algoritmo VSURF eliminou a variável `tmax_mean`, indicando que ela não contribui de forma significativa para o poder preditivo do modelo. 

Dessa forma, o próximo modelo será ajustado considerando apenas as variáveis
selecionadas no objeto `pred_vars`.
...
mostrada acima, ou seja, é sempre importante buscar uma validação robusta para o modelo elaborado. No entanto, a validação anterior  se baseia apenas em uma divisão (aleatória) dos dados em partes de teste e treinamento. Para aprimorar isso, pode-se repetir o processo 
...
Observa-se que a variável ``prec_mean`` tem o maior impacto para δ¹⁵N no solo 
para região de estudo.
...
#Exportando raster
terra::writeRaster(isoscape, filename = here("isoscape", "isoscape_dn15.tif"), overwrite = TRUE)
```
...
points(sites, pch = 21, bg = "black", cex = 0.8)
